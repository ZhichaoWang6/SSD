...
def parse_args():
    ...
    parser.add_argument('--data_path', type=str,
                        default='/data/wangzhichao/projects/SSD/SSD2/datasets/egoexolearn_train.json')
...
def build_loss_mask(input_ids, tokenizer):
    """
    Mark only the LAST assistant turn's tokens with 1.
    (The context turns are supervision-free; only the current response is trained.)
    """
    ...
    for i, token_id in enumerate(ids):
        if token_id == im_start_id:
            in_assistant = False
            if i + 1 < len(ids):
                snippet = tokenizer.decode(ids[i+1:min(i+10, len(ids))],
                                           skip_special_tokens=False)
                if snippet.startswith('assistant'):
                    in_assistant = True
                    region_start = i
        elif token_id == im_end_id and in_assistant:
            assistant_regions.append((region_start, i))
            in_assistant = False

    if assistant_regions:
        start, end = assistant_regions[-1]
