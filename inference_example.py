...
def run_one_sample(model, processor, conversation, args, device, sample_idx=0):
    ...
    speedup = ar_stats['total_time'] / stats['total_time'] if stats['total_time'] > 0 else 0
    ...
    if not output_match:
        print(f"\n  ⚠ WARNING: outputs differ! Greedy decoding should be identical.")
...
def main():
    ...
    parser.add_argument('--adapter_path', type=str,
                        default='/data/wangzhichao/projects/SSD/SSD2/adapter_checkpoints/epoch/adapter_epoch_19')
    ...
    if args.data_path is not None:
        ...
        if len(all_results) > 1:
            ...
            avg_spec_tps = sum(r['spec_tps'] for r in all_results) / len(all_results)
            avg_ar_tps = sum(r['ar_tps'] for r in all_results) / len(all_results)
            print(f"  Output match:      {match_count}/{len(all_results)}")
            print(f"  Avg speedup:       {avg_speedup:.2f}x")
            print(f"  Avg accept length: {avg_accept:.2f}")
            print(f"  Avg spec tok/s:    {avg_spec_tps:.1f}")
            print(f"  Avg AR   tok/s:    {avg_ar_tps:.1f}")
