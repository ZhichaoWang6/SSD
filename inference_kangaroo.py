...
def kangaroo_speculative_generate(
    ...
    verbose: bool = False,  # 新增：显示draft/verify详细过程
):
    ...
    while start_index < max_infer_steps - 1 - speculative_steps:
        ...
        for step in range(1 + speculative_steps):
            ...
            if step == speculative_steps:
                draft_stop_reason = f"达到最大步数 {speculative_steps}"
                break
    ...
    if verbose:
        ...
        print(f"  平均接受长度: {sum(accept_length_list)/len(accept_length_list):.2f}")
